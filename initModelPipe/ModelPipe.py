...
from .doc_analyze_by_init_model import doc_analyze
...
class ModelPipe(AbsPipe):

    def __init__(self, pdf_bytes: bytes, jso_useful_key: dict, image_writer: AbsReaderWriter, ocr_model, txt_model, is_debug: bool = False):
    ...
    def pipe_analyze(self):
        if self.pdf_type == self.PIP_TXT:
            self.model_list = doc_analyze(self.pdf_bytes, model=self.txt_model)
        elif self.pdf_type == self.PIP_OCR:
            self.model_list = doc_analyze(self.pdf_bytes, model=self.ocr_model)
...
if __name__ == '__main__':
    ...
    pipe = ModelPipe(pdf_bytes, jso_useful_key, img_writer)
    pipe.pipe_classify()
    pipe.pipe_parse()
